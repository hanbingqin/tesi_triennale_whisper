...
def record_callback(_, audio:sr.AudioData) -> None:
    """
    Threaded callback function to recDopoeive audio data when recordings finish.
...
while True:
    try:
        ...
        if not data_queue.empty():
            ...
            result = model.transcribe(audio_np, language="it",beam_size=5, fp16=torch.cuda.is_available())
            ...
            if phrase_complete:
                # delay transcription
                delay_end = time.time()
                delay_total= round(delay_end - delay_start, 1)
                #time from last phrase
                since_last = round (  delay_start - end_prev, 1 )
                text = text +"  {transcrizone in "+ str(delay_total) +' s  e tempo passato dall ultima scrittura' + str (since_last) +' s}'
